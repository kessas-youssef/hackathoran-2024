...
function NavBar() {
  ...
  return (
    <nav className='flex justify-center items-center gap-3 sm:gap-5 min-h-24'>
        <img src={waveArrowRight} alt="Wave arrow right" className='navArrow w-8 '/>
        ...
        <NavLink to={'prize'} className={({isActive}) => isActive ? `${styleBtn}`:'text-white'}>Prize</NavLink>
        <NavLink to={'register'} className={({isActive}) => isActive ? `${styleBtn}`:'text-white'}>Register</NavLink>
        <img src={waveArrowLeft} alt="Wave arrow left" className='navArrow w-8'/>
    </nav>
  )
}
